...
func runDemo(ctx context.Context) {
	...
	const (
		minDemoBoxes = 10
		maxDemoBoxes = 60
		pause        = time.Duration(5000 * time.Millisecond)
	)
	...
	for {
		...
		case e < 25: // Set Max TBU to small number and unset expiry
			event.MaxTBU = ptr(2 * pause)
			event.ExpireAfter = new(time.Duration)
		...
		case e < 30: // Unset Max TBU and set expiry
			event.MaxTBU = new(time.Duration)
			event.ExpireAfter = ptr(5 * pause)
